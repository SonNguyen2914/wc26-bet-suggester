...
### Step 8 — execution-quality paper trading (Jul 23)

Roadmap Phase 7 — the execution-strategy evidence layer, deliberately separate from forecast quality, and with *zero* coupling to real money (@bf481c3, migration `755ded7a27ff`, 6th clean prod-PG migration).

`src/live/paper.py` turns each canonical T-10 lock's positive-net-edge contracts into paper decisions, gated on realistic entry conditions (execution-ready snapshot, quote age, executable ask, minimum size, spread, net edge above a floor, approved model). A filled signal is executed by a **deterministic depth-walk** against the *frozen* lock book: buying YES consumes the real NO-bid ladder (`yes_ask = 100 − no_bid`), level by level, with partial fills when depth runs out — never unlimited size at the top — and net-of-fee, net-of-slippage economics. Every fill references its signal, run, contract, and frozen quote (`GET /api/mls/paper`; corpus carries it). Rejections keep their reason, so the ledger has no survivorship bias. `settle_paper` pays 100¢/contract on a hit and records PnL, idempotently, as results land. It is deterministic from the frozen quote+depth stream — the replay acceptance — and `PAPER_TRADING_ENABLED` is a kill switch, not a money gate: `REAL_MONEY_SIGNALS_ENABLED` stays false and untouched.
